...
export async function GET() {
  try {
    ...
    if (existing > 0) {
      return NextResponse.json({ message: `Ya hay ${existing} candidatos. Setup ya fue ejecutado.` })
    }
    ...
    return NextResponse.json({ message: '✅ Setup completado — 7 candidatos cargados correctamente.' })
  } catch (e) {
    console.error(e)
