...
export default function CandidatoTable({ candidatos, onRowClick, onEstadoChange }: Props) {
  ...
  if (!candidatos.length) return <EmptyState message="No hay candidatos con los filtros aplicados." />
  ...
  return (
    <div style={{ overflowX: 'auto' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr>
            {['Candidato', 'Campaña', 'Estado', 'Etapas', 'Riesgo', 'Ops', 'RRHH', 'Cap', 'Alertas'].map(h => (
          ...
          {candidatos.map(c => (
            ...
            >
              <td style={td}>
                ...
                <div style={{ fontSize: 10, color: 'var(--text3)' }}>{c.dni} · {c.fechaPostulacion.split('T')[0]}</div>
              </td>
